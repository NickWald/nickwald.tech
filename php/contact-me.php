...
if($_POST) {
    ...
    $to_Email = "info@nickwald.tech"; // Write your email here
    ...
    if(empty($_POST["userName"])) {
        $output = json_encode(array('type'=>'error', 'text' => '<i class="icon ion-close-round"></i> Sorry but your name is too short or not specified.'));
        die($output);
    }
    ...
    if(strlen($_POST["userMessage"])<10) {
        $output = json_encode(array('type'=>'error', 'text' => '<i class="icon ion-close-round"></i> Your message is too short! It must be at least 10 words long!'));
        die($output);
    }
    ...
    $headers .= 'From: info@nickwald.tech' . "\r\n";
